          ...
          return Column(
            ...
            children: [
              ...
              ...entry.value.map((event) {
                final convertedAmount = settingsCubit.convertFromUAH(
                  event.amount,
                );
...
Map<String, List<EventModel>> groupEventsByMonth(
  ...
  for (final group in grouped.values) {
    group.sort((a, b) {
      final mileageA =
          int.tryParse(a.mileage.replaceAll(RegExp(r'\D'), '')) ?? 0;
      final mileageB =
          int.tryParse(b.mileage.replaceAll(RegExp(r'\D'), '')) ?? 0;
